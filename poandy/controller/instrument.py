...
class InstrumentController(Controller):
    ...
    def get_candles(cls, instrument_name, candle_params={}, df=True):
        ...
        if response.status_code == 200:
            ...
            for price in ['mid', 'bid', 'ask']:
                if price not in candles['candles'].columns: continue
                candles['candles'][f'{price}.open']  = candles['candles'][price].map(lambda x: x['o']).astype(float)
                candles['candles'][f'{price}.high']  = candles['candles'][price].map(lambda x: x['h']).astype(float)
                candles['candles'][f'{price}.low']   = candles['candles'][price].map(lambda x: x['l']).astype(float)
                candles['candles'][f'{price}.close'] = candles['candles'][price].map(lambda x: x['c']).astype(float)
    ...
    def get_orderbook(cls, instrument_name, time=None, df=True):
        ...
        if response.status_code == 200:
            ...
            print(f"Time of order book: {orderbook['time'].strftime('%d/%m/%Y, %H:%M:%S')}")
            print(f"Midpoint price: {orderbook['price']}")
            print(f"Bucket width: {orderbook['bucketWidth']}. Each bucket covers from bucket price to bucket price + bucket width")
    ...
    def get_positionbook(cls, instrument_name, time=None, df=True):
        ...
        if response.status_code == 200:
            ...
            print(f"Time of position book: {positionbook['time'].strftime('%d/%m/%Y, %H:%M:%S')}")
            print(f"Midpoint price: {positionbook['price']}")
            print(f"Bucket width: {positionbook['bucketWidth']}. Each bucket covers from bucket price to bucket price + bucket width    ")
